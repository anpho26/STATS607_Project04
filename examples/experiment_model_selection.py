...
def plot_selection_results(
    ...
    ----------
    results : dict
        Output dictionary from run_experiment.
    scenario_label : str
        Title describing the scenario (e.g., "Experiments with \\epsilon-contamination data").
    filename : str
        File name for saving the figure (e.g., "model_selection_eps_contam.png").
    """
    ...
    # We do not display the figure here; only save it to disk.
    plt.close(fig)


# ============================================================
# 1. CONFIG – FILL THESE NUMBERS FROM THE PAPER
# ============================================================
...
# default configs – tweak to match the text exactly
...
N_REP = 100          # number of replications per n (paper used 100)
...
def rskewnorm(
    ...
    """
    Generate from skew-normal SN(xi, omega, alpha) using Azzalini's
    representation: X = xi + omega * (delta|Z0| + sqrt(1-delta^2) Z1).
    """
    if rng is None:
        rng = np.random.default_rng()
...
def sample_eps_contamination(
    ...
    where p_G is a 2-component Gaussian mixture.
    """
    if rng is None:
        rng = np.random.default_rng()
...
def plot_well_hist_and_dendrogram(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "well_hist_true_density.png",
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "well_dendrogram_k10.png",
                dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_skew_hist_and_dendrogram(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "skew_hist_true_density.png",
                dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "skew_dendrogram_k10.png",
                dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_eps_contam_hist_and_dendrogram(
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "eps_contam_hist_true_density.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "eps_contam_dendrogram_k10.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def fit_overfitted_and_dendrogram(
    ...
    # Convert sklearn GMM to our MixingMeasure (use the helper from our package)
    mm = MixingMeasure.from_sklearn_gmm(gmm)

    # Build dendrogram and return it
...
def select_k_all_criteria(
    ...
    ks_aic = np.array(list(k_range))
    ks_bic = ks_aic

    k_aic = int(ks_aic[np.argmin(aics)])
    k_bic = int(ks_bic[np.argmin(bics)])
...
def run_experiment(
    ...
    Returns dict with counts and averages for AIC, BIC, DIC.
    """
    rng = np.random.default_rng(seed)
    ...
    if setting == "well":
        ...
        generator: Callable[[int, np.random.Generator], np.ndarray] = \
            lambda n, r: sample_gaussian_mixture_1d(
                n, cfg.weights, cfg.means, cfg.sigmas, r
            )
        k0 = cfg.k0
    elif setting == "eps":
        cfg = CFG_EPS
        generator = lambda n, r: sample_eps_contamination(n, cfg, r)
        k0 = cfg.k0
    elif setting == "skew":
        cfg = CFG_SKEW
        generator = lambda n, r: sample_skew_mixture_1d(n, cfg, r)
        k0 = cfg.k0
    else:
    ...
    for i, n in enumerate(n_grid):
        ...
        for rep in range(n_rep):
            ...
            counts_aic.append(k_aic)
            counts_bic.append(k_bic)
            counts_dic.append(k_dic)

        counts_aic = np.asarray(counts_aic)
        counts_bic = np.asarray(counts_bic)
        counts_dic = np.asarray(counts_dic)

        prop_correct_aic[i] = np.mean(counts_aic == k0)
        prop_correct_bic[i] = np.mean(counts_bic == k0)
        prop_correct_dic[i] = np.mean(counts_dic == k0)

        avg_k_aic[i] = counts_aic.mean()
        avg_k_bic[i] = counts_bic.mean()
        avg_k_dic[i] = counts_dic.mean()

        print(
            f"n={n:5d}:  AIC: {prop_correct_aic[i]:.2f}, "
